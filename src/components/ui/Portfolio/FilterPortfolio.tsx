...
const FilterPortfolio = ({
  ...
  if (filterVariant === "reference") {
    ...
    const showServicePills = filter || showModuleTitle;
    const isHomeVariant = showModuleTitle;
    return (
        ...
        <div className="container">
          {showModuleTitle ? (
            <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
              ...
              {showServicePills && (
                <div className="flex flex-wrap gap-2 rounded-2xl border border-border/20 bg-bg p-1">
                  {servicesWithCounts.map(({ label }) => {
                    ...
                    return (
                      <button
                        ...
                        className={`shrink-0 rounded-xl px-4 py-2.5 text-sm font-title font-medium transition-colors cursor-pointer bg-theme text-body shadow-sm ${
                          isSelected
                            ? "bg-theme text-body"
          ...
          <div className="flex flex-col">
            <aside className="relative z-10 flex lg:sticky lg:top-20">
              {showServicePills && (
                <div
                  className={`flex flex-wrap gap-2 rounded-2xl border border-border/20 bg-bg p-1 mb-24 ${
                    showModuleTitle ? "hidden" : ""
                  }`}
                >
                  {servicesWithCounts.map(({ label }) => {
                    const isSelected = selectedService === label;
